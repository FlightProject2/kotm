...
func test_surfaces_water_and_explicit_locked_door() -> void:
	...
	for surface in ["hard", "grass", "wood", "metal", "shallow_water", "deep_water"]:
		...
		await _drive(ch, 45, 0, Vector2(0, 1))
...
func test_exertion_requires_sustained_actual_sprint() -> void:
	...
	await _drive(ch, 510, CharacterInput.B_SPRINT, Vector2(0, 1))
	assert_eq(_count("exertion"), 1, "breath follows actual stamina exhaustion")
	assert_near(Vector2(ch.velocity.x, ch.velocity.z).length(), 2.4, 0.15, "exhaustion slows movement while recovering")
	ch.queue_free(); floor.queue_free()
	await settle(1)
