...
func test_models_merge_and_sockets() -> void:
	...
	assert_true(ModelLib.aabb("ar75").size.x > 0.9, "rifle is about a metre long")
	...
	assert_true(absf(inst.scale.x * ModelLib.aabb("ar75").size.x - 0.75) < 0.01, "fit_to scales the longest side")
	inst.free()

func test_studio_models_reach_the_game() -> void:
	assert_eq(WeaponHolder.MODELS["ar15"], "res://assets/models/ar75.glb")
